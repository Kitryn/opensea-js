...
class OpenSeaAPI {
    ...
    constructor(config, logger) {
        ...
        this.got = config.got ?? got_1.default;
    ...
    async get(apiPath, query = {}) {
        ...
        return JSON.parse(response.body); // typescript hax
    }
    /**
    ...
    async _fetch(apiPath, opts = {}) {
        ...
        // typescript stuff
        const res = await this.got(finalUrl, finalOpts); // typescript hax...
    ...
    async _handleApiResponse(response) {
        ...
        let errorMessage;
        try {
            result = response.body; // typescript hax, can fail if passed in a buffer probs
